	...
	for(p = j->first_process; p; p = p->next) {
		...
		switch (pid = fork()) {
		   ...
		   case 0: /* child */
			...
			errfile = open("mysh.log", O_APPEND | O_CREAT | O_WRONLY, 0666);
			dup2(errfile,2);
			if ((int) j->pgid < 0){
 ...
 * mysh.h. We tried to make the parser flexible but it is not tested
...
char* promptmsg() {
   		const char buf[] = "mysh-";
   		char str [10];
   		char * env_name = "MYSH_PROMPT";
   		sprintf(str, "%s", buf);
        char* first = str;
        char* second = getenv(env_name);
        if(!second)
        	second = "$";
...
int main() {
	...
	dup2(errfile, 2);
	init_shell();
	//allocate memories for job sequence
